@regression @rfq @rfq-vendor @rfq-compare-vendor-price
Feature: RFQ Vendor - compare vendor price after price update

  Create and send an RFQ to a Yopmail vendor, open Yopmail in a new tab, click View RFQ,
  update the vendor price and submit it, then return to the RFQ list and open compare vendor price from the created RFQ card.

  Background:
    Given the RFQ suite is ready with login and Procurement RFQ module open

  Scenario: Compose RFQ, vendor updates price, then compare vendor price
