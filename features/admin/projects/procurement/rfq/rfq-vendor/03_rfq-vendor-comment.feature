@regression @rfq @rfq-vendor @rfq-vendor-comment
Feature: RFQ Vendor - comment

  Create and send an RFQ to a Yopmail vendor, open Yopmail in a new tab, click View RFQ,
  then add a vendor comment from the RFQ vendor portal.

  Background:
    Given the RFQ suite is ready with login and Procurement RFQ module open

  Scenario: Compose RFQ, open Yopmail, vendor adds comment on RFQ
